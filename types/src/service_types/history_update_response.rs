...
use crate::{
    ...
    service_types::HistoryUpdateResult,
};
...
impl BinaryEncoder<HistoryUpdateResponse> for HistoryUpdateResponse {
    ...
    fn decode<S: Read>(stream: &mut S, decoding_options: &DecodingOptions) -> EncodingResult<Self> {
        ...
        let results: Option<Vec<HistoryUpdateResult>> = read_array(stream, decoding_options)?;
        let diagnostic_infos: Option<Vec<DiagnosticInfo>> = read_array(stream, decoding_options)?;
        Ok(HistoryUpdateResponse {
            response_header,
            results,
            diagnostic_infos,
        })
    ...
    fn type_id() -> NodeId {
        ObjectId::HistoryUpdateResponse_Encoding_DefaultBinary.into()
    }
}
